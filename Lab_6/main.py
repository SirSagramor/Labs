...
class RTree:
    def __init__(self, cord, h, w, deapth):
        ...
        self.root = Node(cord, h, w, deapth)
    ...
    def add(self, leaf):
        ...
        # если количество точек больше пяти, то делим коробку на 2 части и добавляем туда точки
            ...
            if self.deapth % 2:
                ...
                self.children[0] = RTree((self.x, (self.y + self.h) / 2), self.w, self.h / 2, self.root.deapth + 1)
                self.children[1] = RTree((self.x, (self.y - self.h) / 2), self.w, self.h / 2, self.root.deapth + 1)
                for i in self.root.leafes:
                    if i.y > self.y:
                        self.children[0].add(i)
                    else:
